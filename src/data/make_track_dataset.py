...
class CovReader(object):
    def __init__(self, options:Dict):
        ...
        self._func = lambda x: np.mean(x, axis=1, dtype=np.float32)
...
def main(mappable:str, tracks:str, reference:str, length:int, width:int, step:int, size:int, batches:int, seed:int, prefix:str) -> int:
    ## Check files exists
    ...
    # ix = np.lexsort((start_pool[ix, 0], start_pool[ix, 1]))
    sites = start_pool[ix, :]

    dataset = open(f"{prefix}.bed", "w")

    for i in range(batches):
        batchix = np.arange(i * size, min((i+1) * size, total_size))
        input = []
        outcome = []
        for ch, start in tqdm.tqdm(sites[batchix]):
            chrom = chrom_ix[ch]
            end = start + length
            
            seq = reference.fetch(chrom, start, end).upper()
            seq = dna_1hot(seq, dtype=np.uint8)
            input.append(seq)
        ...
        np.save(f"{prefix}.input-{i}.npy", np.array(input))
        np.save(f"{prefix}.outcome-{i}.npy", np.array(outcome))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--width', type=int, default=int(os.getenv("SEQ_WINDOW", 128)))
